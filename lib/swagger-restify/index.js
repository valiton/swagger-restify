 ...
 * @return {Function}
 */
exports.init = function (server, opt, passport) {
  // generate resources
  generate(opt);
  ...
  if (opt.swaggerURL && opt.swaggerUI) {
    ...
    server.get(swaggerURL, passport.authenticate('basic', { session: false }), restify.serveStatic({
  ...
  server.get(regex, function (req, res, next) {
    ...
    if (req.params[0]) {
      ...
      resource = resources[req.params[0]];
